...
import org.apache.lucene.util.NotDocIdSet;
...
public class RoaringDocIdSet extends DocIdSet {
    ...
    private static class ShortArrayDocIdSet extends DocIdSet {
        ...
        private ShortArrayDocIdSet(short[] docIDs, boolean invert) {
            this.docIDs = docIDs;
            this.invert = invert;
        }
        ...
        public static ShortArrayDocIdSet read(DataInput input) throws IOException
        {
        ...
        public DocIdSetIterator iterator() throws IOException {
            ...
            DocIdSetIterator iterator = coreIterator();
            if (!invert)
            {
                return iterator;
            }

            int maxDoc = BLOCK_SIZE;
        ...
        public DocIdSetIterator coreIterator() throws IOException {
            return new DocIdSetIterator() {
    ...
    public void write(DataOutput output) throws IOException
    {
        ...
        output.writeVInt(docIdSets.length);

        for (DocIdSet set : docIdSets)
        {
            if (set == null)
            {
                output.writeByte((byte)DocIdSetType.NONE.ordinal());
            }
            else if(set instanceof ShortArrayDocIdSet)
            {
                output.writeByte((byte)DocIdSetType.SHORT_ARRAY.ordinal());
                ShortArrayDocIdSet shortSet = (ShortArrayDocIdSet)set;
                shortSet.write(output);
            }
            else
            {
                output.writeByte((byte)DocIdSetType.BIT.ordinal());
                BitDocIdSet bitDocIdSet = (BitDocIdSet)set;
                FixedBitSet fixedBits = (FixedBitSet)set.bits();
    ...
    public static RoaringDocIdSet read(DataInput input) throws  IOException
    {
        int cardinality = input.readVInt();
        int docIdSetsLength = input.readVInt();

        DocIdSet[] docIdSets = new DocIdSet[docIdSetsLength];
        for (int i = 0; i < docIdSetsLength; i++) {
            DocIdSetType type = DocIdSetType.values()[input.readByte()];
            ...
            else
            {
                ...
                docIdSets[i] = new BitDocIdSet(new FixedBitSet(longs, numBits), cost);
            }
        }
