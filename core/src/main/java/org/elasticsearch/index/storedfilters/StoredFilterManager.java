...
public class StoredFilterManager {
    ...
    public void registerStoredFilter(Engine.Index index, boolean update) {
        ...
        registerStoredFilter(filterName, filterField.query(), doc, update ? index.uid() : null);
    }

    public void registerStoredFilter(String filterName, Query filter, ParseContext.Document document, Term uid) {
    ...
    public void storeFilter(StoredFilterData filterData) {
        ...
        try {
            ...
            for (String segment : docsBySegmentMap.keySet()) {
                ...
                byte[] docBytes = Arrays.copyOfRange(docsOutput.getBytes(), 0, docsOutput.getPosition());
            ...
            if (filterData.uid != null) {
                indexWriter.updateDocument(filterData.uid, filterData.document);
            }
            else {
